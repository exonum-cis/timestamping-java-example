...
class ServiceTransactionTest {
  ...
  @Test
  void hasUniqueIds() {
    ServiceTransaction[] elements = ServiceTransaction.values();

    Multimap<Integer, ServiceTransaction> elementsById = Multimaps.index(Arrays.asList(elements),
        ServiceTransaction::id);

    for (Integer id : elementsById.keySet()) {
      Collection<ServiceTransaction> transactionsWithId = elementsById.get(id);
